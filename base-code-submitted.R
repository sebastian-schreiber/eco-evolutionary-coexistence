...
# returns n11^1/n11^2 and n22^2/n22^1
# both need to be >1 for coexistence
...
blahblah=function(h=1.5){
  blah=function(c1){
   # parms=list(b=1,K=200,d=0.2,delta=c(0.1,0.1),
    #           a=rbind(c(1,2),c(2,1),c(h,h))/25,
...
### THE APPARENT COMPETITION CODE

## THIS NEEDS TO BE DONE!!!

diploid.AC=function(t,n,parms){
  with(parms,{
    n=pmax(n,0)
    p=n[3:5]
    N=sum(p)
    x=p/N
    dn1=r[1]*n[1]*(1-n[1]/K[1])-a[1,1]*n[1]*p[1]-a[1,2]*n[1]*p[2]-a[1,3]*n[1]*p[3]
    dn2=r[2]*n[2]*(1-n[2]/K[2])-a[2,1]*n[2]*p[1]-a[2,2]*n[2]*p[2]-a[2,3]*n[2]*p[3]
    dp1=p[1]*(c[1,1]*a[1,1]*n[1]+c[2,1]*a[2,1]*n[2]+c[3,1]*a[3,1]*n[3]-delta[1])
    dp2=p[2]*(c[1,2]*a[1,2]*n[1]+c[2,2]*a[2,2]*n[2]+c[3,2]*a[3,2]*n[3]-delta[2])
    dn=c(dn1,dn2,dn3,dp1,dp2)
    list(dn)
